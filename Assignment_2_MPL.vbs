...
' EASY SECTION Defining variables
...
' EASY SECTION initially setting volume equal to zero since we're summing over a range
        total_vol = 0
' EASY SECTION looping over the entire sheet
...
' MEDIUM SECTION loops through each sheet to find difference and percentage changes in stock prices
                ...
                close_price_end = 0
' EASY SECTION loops through each sheet to sum the "vol" columns for each ticker -- similar to a "SUMIF" formula
...
' NOT REQUIRED Formatting
        With ActiveWorkbook.Worksheets(i).Range("I1:L1")
            ...
            End With
        End With
' MEDIUM SECTION Formatting -- makes positive changes green and negative changes red
...
' HARD SECTION -- find maximum % increase, % decrease, and total volume
...
' HARD SECTION -- Find Tickers for mins & maxs
        For m = 2 To ActiveWorkbook.Worksheets(i).Range("K1").CurrentRegion.Rows.Count
            ...
            End If
        Next m
' EASY SECTION sets k = 2 so that the loop can start again on a new sheet with the table at the top of the sheet
        k = 2
    Next i
End Sub
